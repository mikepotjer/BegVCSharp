...
namespace GroupQuery
{
    class Program
    {
        static void Main(string[] args)
        {
            ...
            // The data is grouped by a key field, in this case Region. To total for each group, we need
            // to group into a new result set (cg). We can reference the key value in the select using
            // the special group "Key".
            var queryResults =
                from c in customers
                group c by c.Region into cg
                select new { TotalSales = cg.Sum(c => c.Sales), Region = cg.Key };

            // A second query is used simply to sort the results of the first one.
            var orderedResults =
